...
object TeraGen {
    def main(args: Array[String]) {
        ...
        val dataset = sc.parallelize(1 to parts, parts).mapPartitionsWithIndex { case (index, _) =>
            ...
            val rowBytes: Array[Byte] = new Array[Byte](SortInputFormat.RECORD_LENGTH)
            val key = new Array[Byte](SortInputFormat.KEY_LENGTH)
            val value = new Array[Byte](SortInputFormat.VALUE_LENGTH)

            Iterator.tabulate(recordsPerPartition.toInt) { offset =>
                Random16.nextRand(rand)
                generateRecord(rowBytes, rand, recordNumber)
                recordNumber.add(one)
                rowBytes.copyToArray(key, 0, SortInputFormat.KEY_LENGTH)
                rowBytes.takeRight(SortInputFormat.VALUE_LENGTH).copyToArray(value, 0,
